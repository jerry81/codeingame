...
struct POS {
    int x, y;
    POS &operator+=(const POS &other) { x += other.x; y += other.y; return *this; } // leet: operator
    bool operator==(const POS &other) const { return (x == other.x) && (y == other.y); }
};
inline POS operator+(POS p1, const POS& p2) { return p1 += p2; } // leet more operators
inline bool operator!=(const POS& p1, const POS& p2) { return !(p1 == p2); }


template <typename T> // leet: template
...
char* DIR_TO_STRINGZ[] = { "SOUTH", "EAST", "NORTH", "WEST" }; // leet, char*
...
struct ROBOT_STATE {
    ...
    bool operator==(const ROBOT_STATE& other) {
        return !memcmp(this, &other, sizeof(*this)); // leet memcmp
    }
};
...
public:
    ...
    int Wander(void)
    {
        ...
        switch(at(city_map, new_pos)) // so we use switch still
        {
            case ' ': case '@': break; // multiple cases on a single line
            case 'X':
                if (state.breaker) {
...
int main()
{
    ...
    try { // leet: try catch
        ...
        while(Bender.is_alive)
        {
            ...
            Bender.ImprintState();  // camel case

            Output.push_back(Bender.Wander());
        }
    }
    catch (typeof("") ex) { // leet, typeof
        cerr << ex << endl; exit(0);
    }

    for(auto &i : Output) { // leet, auto
