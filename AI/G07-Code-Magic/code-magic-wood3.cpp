...
the real challenge will begin. The Goal Draft a deck of cards, battle an
opponent with those cards and reduce their Health Points (HP) from 30 to 0.
        Rules
This game is a two-player card game which is played in two phases: the Draft
phase and the Battle phase. During the Draft phase, both players must create a
deck of 30 cards. Once the Draft phase is over, both decks are shuffled. During
the Battle, the board is divided in two parts: each player plays cards from
their hand on their side of the board. To reduce the health points of an
opponent, the player must make use of cards to deal damage. Draft Phase For 30
turns, both players are given a choice between 3 different cards. Players select
the card they want to add to their deck by using the PICK command followed by 0,
1 or 2. By default, the PASS command will pick the first card. Both players can
select the same card, they will each receive a copy. Battle Phase Card Draw
...
Both players possess runes which have an impact on card draw. However, runes
should be ignored until the Bronze league.
...
points and defense points. By default, creatures can't attack the turn they are
summoned. They can attack once per turn only. When a creature attacks another
one, they both deal damage equals to their attack to the defense of the other
creature. When a creature attacks the opponent, it deals damage equals to its
attack to the HP of the opponent. Creatures are removed from play when their
defense reaches 0 or less. Gameplay Possible Actions SUMMON id to summon the
creature id from your hand. ATTACK id1 id2 to attack creature id2 with creature
id1. ATTACK id -1 to attack the opponent directly with creature id. PASS to do
nothing this turn. A player can do any number of valid actions during one turn.
Actions must be separated by a semi-colon ;.
...
in this league Integer playerDraw: to be ignored in this league. The player's
input comes first, the opponent's input comes second.
